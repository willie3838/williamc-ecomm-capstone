...
| **`hillclimb`** | [hillclimb/](file:///usr/local/google/home/williamwlchan/Playground/williamc-ecomm-capstone/skills/hillclimb/SKILL.md) | Autonomous iterative optimization: run evals/tests, identify failures, hypothesize root causes, implement fixes, and re-verify until complete. | `scripts/run_checks.sh`<br/>`scripts/run_smoke_eval.py`<br/>`resources/mock_catalog_fixture.py`<br/>`resources/eval_criteria.json` |
